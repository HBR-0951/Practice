...
int input(int max, int a[],unsigned int b[]){
...
void check(int k,int a[],unsigned int b[]){
	int c=0;
	int count=0; 
	for(int i=0;i<k;i++){
...
void switch1(unsigned int *a,unsigned int *b){
...
int main(){
	...
	int max1,max2,k,h,a[1000],c[1000],l,time=0,count1,count2,len;
	scanf("%d",&max1);
	...
	int e[k*h];
	unsigned int f[k*h];		//把陣列a,b合併成陣列c 
	int q=0;
	for(int i=0;i<k;i++){
		for(int j=0;j<h;j++){
			e[q]=a[i]*c[j];
